...
void CCJMenu::DrawItem_Win9xNT2000 (LPDRAWITEMSTRUCT lpDIS)
{
	...
	if(state & MF_SEPARATOR){
		...
		rect.left = rect.left + m_iconX + 8 + CCJMenu_GAP; 
...
COLORREF CCJMenu::LightenColor(COLORREF col,double factor)
{
...
COLORREF CCJMenu::DarkenColor(COLORREF col,double factor)
{
	if(factor>0.0&&factor<=1.0){
		BYTE red,green,blue,lightred,lightgreen,lightblue;
		red = GetRValue(col);
...
void CCJMenu::DrawItem_WinXP (LPDRAWITEMSTRUCT lpDIS)
{
	ASSERT(lpDIS != NULL);
	CDC* pDC = CDC::FromHandle(lpDIS->hDC);
#ifdef CCJMenu_USE_MEMDC
	CCJMenuMemDC *pMemDC=NULL;
#endif
	CRect rect,rect2;
	UINT state = (((CCJMenuData*)(lpDIS->itemData))->nFlags);
	...
	int CCJMenu_PAD=4;
	if(xp_draw_3D_bitmaps)CCJMenu_PAD=7;
	int barwidth=m_iconX+CCJMenu_PAD;
	
	// remove the selected bit if it's grayed out
	if(lpDIS->itemState & ODS_GRAYED&&!xp_select_disabled){
		if(lpDIS->itemState & ODS_SELECTED)lpDIS->itemState=lpDIS->itemState & ~ODS_SELECTED;
#ifdef CCJMenu_USE_MEMDC
		pMemDC=new CCJMenuMemDC(pDC,&lpDIS->rcItem);
		pDC = pMemDC;
	...
	if(state & MF_SEPARATOR){
		...
		rect.top+=rect.Height()>>1;
		rect.left = rect2.right+CCJMenu_PAD;
		pDC->DrawEdge(&rect,EDGE_ETCHED,BF_TOP);
...
void CCJMenu::MeasureItem( LPMEASUREITEMSTRUCT lpMIS )
{
	...
	int CCJMenu_PAD=4;
	if(IsLunaMenuStyle()&&xp_draw_3D_bitmaps)CCJMenu_PAD=7;
	if(state & MF_SEPARATOR){
		lpMIS->itemWidth = 0;
		...
		if(IsLunaMenuStyle())
			...
			lpMIS->itemHeight = temp>(m_iconY+CCJMenu_PAD)/2 ? temp : (m_iconY+CCJMenu_PAD)/2;
	}
	else{
		CFont m_fontMenu;
		LOGFONT m_lf;
		...
		if(IsLunaMenuStyle())lpMIS->itemWidth = m_iconX+CCJMenu_PAD+8+t.cx;
		else lpMIS->itemWidth = m_iconX + t.cx + m_iconX + CCJMenu_GAP;
		int temp = GetSystemMetrics(SM_CYMENU);
		lpMIS->itemHeight = temp>m_iconY+CCJMenu_PAD ? temp : m_iconY+CCJMenu_PAD;
		m_fontMenu.DeleteObject();
	}
}
                           ...
                           int nIconNormal)
{
	if(!(nFlags & MF_BYPOSITION)){
		int iPosition =0;
		CCJMenu* pMenu = FindMenuOption(nPosition,iPosition);
		if(pMenu){
...
BOOL CCJMenu::ModifyODMenuA(const char * lpstrText,UINT nID,COLORREF fill,COLORREF border,int hatchstyle,CSize *pSize)
{
	USES_CONVERSION;
	return ModifyODMenuW(A2W(lpstrText),nID,fill,border,hatchstyle,pSize);
}

BOOL CCJMenu::ModifyODMenuW(wchar_t *lpstrText,UINT nID,COLORREF fill,COLORREF border,int hatchstyle,CSize *pSize)
{
	CWnd *pWnd = AfxGetMainWnd();            // Get main window
	CDC *pDC = pWnd->GetDC();              // Get device context
	SIZE sz;
...
CCJMenuData *CCJMenu::FindMenuItem(UINT nID)
{
	...
	if (!pData){
		int loc;
		CCJMenu *pMenu = FindMenuOption(nID, loc);
		ASSERT(pMenu != this);
		if (loc >= 0){
...
CCJMenu *CCJMenu::FindAnotherMenuOption(int nId,int& nLoc,CArray<CCJMenu*,CCJMenu*>&bcsubs,
									  CArray<int,int&>&bclocs)
{
	int i,numsubs,j;
...
BOOL CCJMenu::LoadMenu(HINSTANCE hInstance, int nResource)
{
	return(CCJMenu::LoadMenu(hInstance, MAKEINTRESOURCE(nResource)));
}
...
BOOL CCJMenu::LoadMenu(LPCTSTR lpszResourceName)
{
	...
	if (NULL != hInst)
	{
		return CCJMenu::LoadMenu(hInst, lpszResourceName);
	}
	return FALSE;
}


BOOL CCJMenu::LoadMenu(HINSTANCE hInst, LPCTSTR lpszResourceName)
{
	ASSERT_VALID(this);
	ASSERT(lpszResourceName != NULL);
...
void CCJMenu::RemoveTopLevelOwnerDraw(void)
{
	CString str;
	int i=0,j=0;
...
void CCJMenu::DrawCheckMark(CDC* pDC,int x,int y,COLORREF color,BOOL narrowflag)
{
	int dp=0;
	CPen m_penBack;
...
void CCJMenu::GetFadedBitmap(CBitmap &bmp)
{
	CDC ddc;
	COLORREF bgcol,col;
	BITMAP BitMap;
...
void CCJMenu::GetTransparentBitmap(CBitmap &bmp)
{
	CDC ddc;
...
void CCJMenu::GetDisabledBitmap(CBitmap &bmp,COLORREF background)
{
	CDC ddc;
...
void CCJMenu::GetShadowBitmap(CBitmap &bmp)
{
	CDC ddc;
...
BOOL CCJMenu::DrawXPCheckmark(CDC *dc, const CRect& rc, HBITMAP hbmCheck,COLORREF &colorout,BOOL selected)
{
	CBrush brushin;
	COLORREF col;
	int dx,dy;
...
void CCJMenu::DitherBlt3(CDC *drawdc, int nXDest, int nYDest, int nWidth, 
...
WORD CCJMenu::NumBitmapColors(LPBITMAPINFOHEADER lpBitmap)
{
...
BOOL CCJMenu::ImageListDuplicate(CImageList *il,int xoffset,CImageList *newlist)
{
...
CMenu* CCJMenu::GetSubMenu(int nPos)
{
	return CMenu::GetSubMenu (nPos);
}

CMenu* CCJMenu::GetSubMenu(LPCTSTR lpszSubMenuName)
{
...
CCJMenu* CCJMenu::GetSubCCJMenu(char* pText)
{
	USES_CONVERSION;
	return GetSubCCJMenu(A2W(pText));
}

CCJMenu* CCJMenu::GetSubCCJMenu(wchar_t* lpszSubMenuName)
{
	CCJMenuData *mdata;
...
int CCJMenu::GetMenuPosition(char* pText)
{
	USES_CONVERSION;
	return GetMenuPosition(A2W(pText));
}

int CCJMenu::GetMenuPosition(wchar_t* pText)
{
	int i,j;
	CCJMenu* psubmenu;
	for(i=0;i<(int)(GetMenuItemCount());++i)
...
int CCJMenu::RemoveMenu(char* pText, BC_Seperator sPos)
{
	USES_CONVERSION;
	return RemoveMenu(A2W(pText), sPos);
}

int CCJMenu::RemoveMenu(wchar_t* pText, BC_Seperator sPos)
{
	...
	if(sPos == CCJMenu_HEAD)
	{
		ASSERT(nPos - 1 >= 0);
		RemoveMenu(nPos-1, MF_BYPOSITION);
	}
	else if(sPos == CCJMenu_TAIL)
	{
		RemoveMenu(nPos-1, MF_BYPOSITION);
	}
	else if(sPos == CCJMenu_BOTH)
	{
...
int CCJMenu::DeleteMenu(char* pText, BC_Seperator sPos)
{
	USES_CONVERSION;
	return DeleteMenu(A2W(pText), sPos);
}

int CCJMenu::DeleteMenu(wchar_t* pText, BC_Seperator sPos)
{
	int nPos = GetMenuPosition(pText);
	if(nPos != -1)
	{
...
// Larry Antram
BOOL CCJMenu::SetMenuText(UINT id, CString string, UINT nFlags/*= MF_BYPOSITION*/ )
{
	BOOL returnflag=FALSE;
...
// courtesy of Warren Stevens
void CCJMenu::ColorBitmap(CDC* pDC,CBitmap& bmp,CSize bitmap_size,CSize icon_size,COLORREF fill,COLORREF border,int hatchstyle)
{
	CDC bmpdc;
	COLORREF m_newclrBack;
...
BOOL CCJMenu::IsWindowsClassicTheme(void)
{
...
int CCJMenu::GlobalImageListOffset(int nID)
{
...
BOOL CCJMenu::CanDraw3DImageList(int offset)
{
	BOOL retflag=FALSE;
	int numcurrent=m_AllImagesID.GetSize();
...
int CCJMenu::AddToGlobalImageList(CImageList *il,int xoffset,int nID)
{
	int loc = -1;
