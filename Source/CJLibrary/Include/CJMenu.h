 ...
 * $Date: 13/12/02 23:11 $
 * $Revision: 5 $
 * $Archive: /Projects/Libraries/CJLibrary/Include/CJMenu.h $
 *
 * $History: CJMenu.h $
 * 
 * *****************  Version 5  *****************
 * User: Anna         Date: 13/12/02   Time: 23:11
 * Updated in $/Projects/Libraries/CJLibrary/Include
 * Added new overloads of LoadMenu() to allow the resource handle to be
 * specified
 * 
 *
 * *****************  Version 4  *****************
 * User: Andy         Date: 10/05/02   Time: 14:09
 * Updated in $/Projects/Libraries/CJLibrary/Include
 * Integrated BCMenu 3.03
 * 
 * *****************  Version 5  *****************
 * User: Ajm          Date: 27/02/02   Time: 17:19
 * Updated in $/Third Party Software/CJLibrary 6.09/Include
 * Integrated BCMenu 3.01
 * 
 * *****************  Version 3  *****************
 * User: Andy         Date: 2/26/02    Time: 4:14p
 * Updated in $/Projects/Libraries/CJLibrary/Include
 * Integrated BCMenu 3.01
 * 
 * *****************  Version 2  *****************
 * User: Andy         Date: 1/04/02    Time: 10:54p
...
public:
	CCJMenuData () {menuIconNormal=-1;xoffset=-1;bitmap=NULL;pContext=NULL;
...
typedef enum {CCJMenu_DRAWMODE_ORIGINAL,CCJMenu_DRAWMODE_XP} BC_MenuDrawMode;

// how seperators are handled when removing a menu (Tongzhe Cui)
typedef enum {CCJMenu_NONE, CCJMenu_HEAD, CCJMenu_TAIL, CCJMenu_BOTH} BC_Seperator;
...
class _CJX_EXT_CLASS CCJMenu : public CMenu
{
	DECLARE_DYNAMIC( CCJMenu )
public:
	CCJMenu(); 
	virtual ~CCJMenu();
	...
	virtual BOOL LoadMenu(HINSTANCE hInstance, LPCTSTR lpszResourceName);
	virtual BOOL LoadMenu(HINSTANCE hInstance, int nResource);
	...
	int RemoveMenu(char* pText, BC_Seperator sPos=CCJMenu_NONE);
	int RemoveMenu(wchar_t* pText, BC_Seperator sPos=CCJMenu_NONE);
	int DeleteMenu(char* pText, BC_Seperator sPos=CCJMenu_NONE);
	int DeleteMenu(wchar_t* pText, BC_Seperator sPos=CCJMenu_NONE);
	...
	CCJMenu* GetSubCCJMenu(char* lpszSubMenuName);
	CCJMenu* GetSubCCJMenu(wchar_t* lpszSubMenuName);
	...
	static void SetMenuDrawMode(UINT mode){
		CCJMenu::original_drawmode=mode;
		CCJMenu::xp_drawmode=mode;
	};
	...
	static void SetSelectDisableMode(BOOL mode){
		CCJMenu::original_select_disabled=mode;
		CCJMenu::xp_select_disabled=mode;
	};
	...
	static void SetXPBitmap3D(BOOL val){
		CCJMenu::xp_draw_3D_bitmaps=val;
	};
	static BOOL GetXPBitmap3D(void){return CCJMenu::xp_draw_3D_bitmaps;}
...
protected:
	...
	CCJMenu *FindAnotherMenuOption(int nId,int& nLoc,CArray<CCJMenu*,CCJMenu*>&bcsubs,
...
#define CCJMenu_USE_MEMDC

#ifdef CCJMenu_USE_MEMDC
//////////////////////////////////////////////////
// CCJMenuMemDC - memory DC
//
// Author: Keith Rule
...
class _CJX_EXT_CLASS CCJMenuMemDC : public CDC
{
public:

    // constructor sets up the memory DC
    CCJMenuMemDC(CDC* pDC,LPCRECT lpSrcRect) : CDC()
    {
        ASSERT(pDC != NULL);
    ...
    CCJMenuMemDC* operator->() {return this;}
...
private:
    ...
    CBitmap* m_pOldBitmap;  // bitmap originally found in CCJMenuMemDC
